...
test('migration adds linea column numbered per factura', async () => {
  ...
  await store.init([
    `CREATE TABLE "Factura_Items" ("id_factura" TEXT, "linea" INTEGER, "descripcion" TEXT, "cantidad" REAL, "precio_unitario" REAL, "importe" REAL, "id_producto" TEXT, PRIMARY KEY ("id_factura", "linea"))`
  ])
  ...
  await store.replaceTable('Factura_Items', [
    ...
    { id_factura: 'F-001', linea: -2, descripcion: 'B', cantidad: 2, precio_unitario: 50, importe: 100, id_producto: '' },
    { id_factura: 'F-002', linea: -1, descripcion: 'C', cantidad: 1, precio_unitario: 200, importe: 200, id_producto: '' }
  ])
...
test('migration orders by rowid within each factura', async () => {
  ...
  await store.replaceTable('Factura_Items', [
    { id_factura: 'F-001', linea: -1, descripcion: 'FIRST', cantidad: 1, precio_unitario: 100, importe: 100, id_producto: '' },
    { id_factura: 'F-001', linea: -2, descripcion: 'SECOND', cantidad: 1, precio_unitario: 100, importe: 100, id_producto: '' },
    { id_factura: 'F-001', linea: -3, descripcion: 'THIRD', cantidad: 1, precio_unitario: 100, importe: 100, id_producto: '' },
  ])
