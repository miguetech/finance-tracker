...
export type EsquemaPago = 'weekly' | 'biweekly' | 'monthly'
export type PagoDividido = { payment_method: string; currency: string; amount: number }
...
export function totalNomina(input: {
  ...
  return round2(input.base_salary + he + (input.bonos ?? 0) + (input.fees ?? 0))
}
...
export function consolidadoNomina(empleados: Empleado[], detalles: NominaDetalle[], gastos: Gasto[], mes: string): {
  ...
  for (const emp of empleados) {
    ...
    if (det) {
      ...
      totCom += Number(det.fees) || 0
      tot += Number(det.total) || 0
    }
    lineas.push({ empleado: emp, detail: det, gasto })
  }
  return {
    lineas,
...
export function desgloseEmpleado(
  empleado: Empleado,
  datos: { asistencias: { date: string; clock_in: string; clock_out: string }[] }
    & { detalles?: NominaDetalle[]; expenses?: Gasto[] },
  mes: string
): DesgloseEmpleado {
  let dias = 0
  let horas = 0
  for (const a of datos.asistencias.filter(a => a.date.slice(0, 7) === mes)) {
    dias++
    horas += horasEntre(a.clock_in, a.clock_out)
  }
  const det = datos.detalles?.find(d => d.employee_id === empleado.employee_id && d.mes === mes) ?? null
  const pagados = (datos.expenses ?? [])
    .filter(g => g.category === 'Nómina' && String(g.supplier) === empleado.name && g.date.slice(0, 7) === mes)
