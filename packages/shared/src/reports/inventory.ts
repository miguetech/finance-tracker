...
export function productosStockBajo(productos: Producto[]): ProductoBajoStock[] {
  return productos
    .filter(p => p.active !== 'false' && Number(p.minimum_stock) > 0 && Number(p.stock) <= Number(p.minimum_stock))
    .map(p => ({
...
export function movimientosPorMes(movimientos: MovimientoStock[], rango: RangoFecha): MovimientosMes[] {
  ...
  for (const m of movimientos) {
    ...
    const mes = monthKey(m.date)
    const row = map.get(mes) ?? { mes, entradas: 0, salidas: 0 }
    if (m.type === 'in') row.entradas = round2(row.entradas + m.quantity)
    else row.salidas = round2(row.salidas + m.quantity)
    map.set(mes, row)
  }
...
export function statsMultiproducto(input: InputMultiproducto): StatsProducto[] {
  ...
  // Facturas del rango con su currency/tipo_cambio para convertir a base.
  ...
  for (const it of items) {
    ...
    if (!fac) continue
    vendidas.set(pid, round2((vendidas.get(pid) ?? 0) + it.quantity))
  ...
  for (const m of movimientos) {
    if (!idSet.has(m.product_id) || m.type !== 'out') continue
    if ((rango.desde && m.date.slice(0, 10) < rango.desde) || (rango.hasta && m.date.slice(0, 10) > rango.hasta)) continue
    salidaPeriodo.set(m.product_id, round2((salidaPeriodo.get(m.product_id) ?? 0) + m.quantity))
  }
  ...
  for (const pid of ids) {
    ...
    rows.push({
      ...
      velocidad_out: dias > 0 ? roundTo(salidas / dias, 2) : 0
    })
  }
...
export function ventasRapidas(productos: Producto[], movimientos: MovimientoStock[], rango: RangoFecha, topN = 10): StatsProducto[] {
  return statsMultiproducto({
    ...
    .filter(r => r.velocidad_out > 0)
    .sort((a, b) => b.velocidad_out - a.velocidad_out)
    .slice(0, topN)
}

function enRangoF(date: string, r: RangoFecha): boolean {
