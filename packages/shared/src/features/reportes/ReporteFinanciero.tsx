...
function ExportButtonsFin({ data, desde, hasta, moneda, empresa, titulo }: { data: FinData; desde: string; hasta: string; moneda: string; empresa: string; titulo: string }) {
  const { t } = useI18n()
  const nombre = `${titulo || 'reporte'}_${desde}_${hasta}`.replace(/\s+/g, '_')
  const subtitulo = `${desde}${hasta ? ` → ${hasta}` : ''}`

  const csvPL = () => exportCSV(nombre, [
  ...
  const pdfAll = () => exportPDF(titulo || t('reportes.title'), {
    ...
    subtitulo,
    cards: [
      { label: t('reportesFin.ingresosTotales'), value: formatMoney(data.pl.ingresos_totales, moneda) },
      { label: t('reportesFin.utilidadNeta'), value: formatMoney(data.pl.utilidad_neta, moneda), tone: data.pl.utilidad_neta >= 0 ? 'pos' : 'neg' },
      { label: t('reportesFin.puntoEquilibrio'), value: data.equilibrio.punto_equilibrio < 0 ? '∞' : formatMoney(data.equilibrio.punto_equilibrio, moneda) }
    ],
    tablas: [
      {
        titulo: t('reportesFin.tabPL'),
        columnas: ['Concepto', `Monto (${moneda})`],
        numericas: [1],
        filas: [
      ...
      }] : []),
      {
        ...
        numericas: [1, 2, 3],
        filas: data.flujo.porMoneda.map(m => [m.moneda, m.entradas, m.salidas, m.balance])
      }
    ]
  })
...
function TabPL({ data, moneda }: { data: FinData; moneda: string }) {
  const { t } = useI18n()
  const pl = data.pl
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <StatCard label={t('reportesFin.ingresosTotales')} value={formatMoney(pl.ingresos_totales, moneda)} tone="positive" />
        <StatCard label={t('reportesFin.costoMercancia')} value={formatMoney(pl.costo_mercancia, moneda)} tone="negative" />
        <StatCard label={t('reportesFin.gastosFijosLbl')} value={formatMoney(pl.gastos_fijos, moneda)} tone="negative" />
        <StatCard label={t('reportesFin.gastosVariables')} value={formatMoney(pl.gastos_variables, moneda)} tone="negative" />
        <StatCard label={`${t('reportesFin.utilidadNeta')} (${pl.margen_neto_pct}%)`} value={formatMoney(pl.utilidad_neta, moneda)} tone={pl.utilidad_neta >= 0 ? 'positive' : 'negative'} />
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card title={t('reportesFin.lineasCostos')}>
          <HBarChart data={pl.lineas_costos.slice(0, 10).map(l => ({ label: l.concepto, value: l.monto }))} format={(n) => formatMoney(n, moneda)} color="#ef4444" />
        </Card>
        <Card title={t('dashboard.facturado')}>
          <DonutChart data={pl.lineas_ingresos.map(l => ({ label: l.concepto, value: l.monto }))} format={(n) => formatMoney(n, moneda)} />
        </Card>
      </div>
    </div>
  )
...
function TabEquilibrio({ data, moneda }: { data: FinData; moneda: string }) {
  const { t } = useI18n()
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
        <Card title={t('reportesFin.cobertura')}>
          <Gauge pct={eq.cobertura_pct} label={`${Math.min(999, Math.round(eq.cobertura_pct))}%`} sublabel={eq.rentable ? t('reportesFin.rentable') : t('reportesFin.noRentable', { faltante: formatMoney(faltante, moneda) })} />
        </Card>
        <Card title={t('reportesFin.tabEquilibrio')}>
          <dl className="space-y-2 text-sm">
            <Row label={t('reportesFin.costosFijos')} value={formatMoney(eq.costos_fijos, moneda)} />
            <Row label={t('reportesFin.costosVariables')} value={formatMoney(eq.costos_variables, moneda)} />
            <Row label={t('reportesFin.ingresosTotales')} value={formatMoney(eq.ingresos, moneda)} />
            <Row label={t('reportesFin.margenContribucion')} value={`${eq.margen_contribucion_pct}%`} />
            <div className="border-t pt-2"><Row strong label={t('reportesFin.puntoEquilibrio')} value={Number.isFinite(eq.punto_equilibrio) ? formatMoney(eq.punto_equilibrio, moneda) : '∞'} /></div>
          </dl>
        </Card>
        <Card title={t('reportesFin.perdidasVsGanancias')}>
          <BarChart height={180}
            ...
            format={(n) => formatMoney(n, moneda)} color="#10b981"
          />
          <div className="mt-3">
...
function TabReconversion({ data, moneda, desde, hasta, empresa }: { data: FinData; moneda: string; desde: string; hasta: string; empresa: string }) {
  const { t } = useI18n()
  ...
  const cols = useMemo(() => [
    ...
    { key: 'monto_moneda', header: t('common.monto'), render: (row: Record<string, unknown>) => String(row.monto_moneda) },
  ...
  const pdfRecon = () => exportPDF(t('reportesFin.tabReconversion'), {
    ...
    cards: [
      { label: t('reportesFin.perdidaTotal'), value: formatMoney(r.perdida_total, moneda), tone: 'neg' },
      { label: t('reportesFin.gananciaTotal'), value: formatMoney(r.ganancia_total, moneda), tone: 'pos' },
      { label: t('reportesFin.impactoNeto'), value: formatMoney(r.neto, moneda), tone: r.neto >= 0 ? 'pos' : 'neg' }
    ],
    tablas: [{
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard label={t('reportesFin.perdidaTotal')} value={formatMoney(r.perdida_total, moneda)} tone="negative" />
        <StatCard label={t('reportesFin.gananciaTotal')} value={formatMoney(r.ganancia_total, moneda)} tone="positive" />
        <StatCard label={t('reportesFin.impactoNeto')} value={formatMoney(r.neto, moneda)} tone={r.neto >= 0 ? 'positive' : 'negative'} />
      </div>
          ...
          <tbody>
            {(rows as Record<string, unknown>[]).map((v, i) => (
              <tr key={i} className="border-t border-gray-100 hover:bg-muted/50">
                <td className="px-4 py-2 whitespace-nowrap">{String(v.fecha)}</td>
                <td className="px-4 py-2 max-w-56 truncate" title={String(v.descripcion)}>{String(v.descripcion)}</td>
                <td className="px-4 py-2"><Badge tone="gray">{String(v.tipo)}</Badge></td>
                <td className="px-4 py-2">{String(v.moneda)}</td>
                <td className="px-4 py-2 text-right tabular-nums">{String(v.monto_moneda)}</td>
...
function TabFlujo({ data, moneda, desde, hasta, empresa }: { data: FinData; moneda: string; desde: string; hasta: string; empresa: string }) {
  const { t } = useI18n()
  const f = data.flujo
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <StatCard label={t('reportesFin.totalEntradas')} value={formatMoney(f.totalEntradasBase, moneda)} tone="positive" />
        <StatCard label={t('reportesFin.totalSalidas')} value={formatMoney(f.totalSalidasBase, moneda)} tone="negative" />
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card title={t('reportesFin.porMoneda')} footer={<div className="flex justify-end"><Button size="sm" variant="outline" onClick={exportCSVFlujo}>{t('reportesFin.exportarCSV')}</Button></div>}>
          <table className="w-full text-sm">
            ...
            <tbody>
              {f.porMoneda.map(m => (
                <tr key={m.moneda} className="border-t border-gray-50">
                  <td className="py-2 font-medium">{m.moneda}</td>
