...
export function AlmacenamientoCard() {
  ...
  const inventarioQ = useQuery({ queryKey: ['inventory'], queryFn: () => repo.inventarioHojas() })
  ...
  const verificar = async (id: string) => {
    ...
    try {
      ...
      toast(`${t('almacen.verificar')}: ${t(`almacen.rol.${r.status}` as never)}${r.motivo ? ` (${t(`almacen.rol.${r.motivo}` as never)})` : ''}`, 'success')
    } catch (e) {
      toast((e as Error).message, 'error')
    } finally {
      setVerificando(null)
  ...
  const identidad = (h: HojaInfo | undefined) => !h ? null : (
    <div className="flex flex-wrap items-center gap-1.5">
      <Badge className={h.type === 'base' ? 'bg-blue-50 text-blue-700' : h.type === 'eventos' ? 'bg-indigo-50 text-indigo-700' : 'bg-gray-100 text-gray-500'}>
        {h.type === 'base' ? t('almacen.tipoBase') : h.type === 'eventos' ? t('almacen.tipoEventos') : t('almacen.tipoLegacy')}
      </Badge>
      <Badge className={h.status === 'reemplazado' ? 'bg-red-50 text-red-600' : h.status === 'borrado' ? 'bg-gray-200 text-gray-500' : h.status === 'legacy' ? 'bg-gray-100 text-gray-500' : 'bg-green-50 text-green-700'}>
        {h.status === 'reemplazado' ? t('almacen.reemplazado') : h.status === 'borrado' ? t('almacen.borrado') : h.status === 'legacy' ? t('almacen.tipoLegacy') : t('almacen.activo')}
      </Badge>
            ...
            <ul className="divide-y divide-gray-50 rounded-lg border border-gray-100 bg-white px-3 py-1">
              {preflight.hojas.map(h => (
                <li key={h.id} className="flex items-center justify-between gap-2 py-1.5">
                  <span className="text-xs truncate">{h.type === 'base' ? t('almacen.tipoBase') : `${t('almacen.tipoEventos')} ${h.año}`}</span>
                  <Badge className={ROL_CLASS[h.rol]}>{t(`almacen.rol.${h.rol}` as never)}</Badge>
                </li>
              ))}
            </ul>
