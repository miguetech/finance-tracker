...
import { printInvoice } from '../invoices/FacturaDetail'
...
export function Configuracion() {
  ...
  if (!form.company_name.trim()) errors.name = t('errors.nombreObligatorio')
  if (!String(form.serial_prefix).trim()) errors.prefijo = t('errors.prefijoObligatorio')
  if (String(form.vat_percent).trim() === '' || isNaN(numIva) || numIva < 0 || numIva > 100) errors.vat = t('errors.ivaRango')
  ...
  const sampleFactura = (): { factura: Factura; items: FacturaItem[] } => {
    const items = [{ description: 'Concepto de ejemplo', quantity: 1, unit_price: 100, importe: 100 }]
    const subtotal = 100
    const vat = Math.round(subtotal * (numIva || 0)) / 100
    return {
      factura: {
        invoice_id: 'preview', serial: folioPreview, customer_id: '', customer_name: 'Cliente de ejemplo',
        issue_date: hoy, due_date: '', subtotal, vat, total: subtotal + vat,
        balance: subtotal + vat, paid_at: '', notes: 'Factura de ejemplo', currency: form.currency, exchange_rate: 1, edited: '', edited_at: ''
      },
      items
    }
  }
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (logoFile) {
        ...
        logo = await repo.uploadImage({ name: `logo_${Date.now()}.${mimeType.split('/')[1] || 'png'}`, mimeType, base64, modulo: 'settings' })
      }
    ...
    <div className="space-y-6">
      ...
      <Card title={t('configuracion.datosEmpresa')}>
        <div className="space-y-3">
          <div><label className="text-xs text-muted-foreground">{t('configuracion.empresaNombre')} *</label><Input value={form.company_name} onChange={set('empresa_nombre')} error={errors.name} /></div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              {ayudaFolio && (
                <div className="mt-2 rounded-lg border border-gray-100 bg-muted/50 p-3 text-xs text-muted-foreground space-y-1.5">
                  {FOLIO_TOKENS.map(tk => (
                    <div key={tk.token} className="flex justify-between gap-3"><span className="font-mono text-gray-700">{tk.token}</span><span>{tk.description} — ej. {tk.ejemplo}</span></div>
                  ))}
            ...
            <div>
              ...
              <Input type="number" min={0} max={100} step="any" value={form.vat_percent} onChange={set('iva_porcentaje')} error={errors.vat} />
            </div>
            <div>
              <div className="flex items-center gap-1.5 mb-1">
              ...
              <Select value={form.currency} onChange={v => setForm(f => f && ({ ...f, currency: v }))} options={CURRENCIES.map(c => ({ value: c.code, label: `${c.code} · ${c.name}` }))} />
            </div>
          </div>
          <div className="text-sm text-muted-foreground">{t('configuracion.proximaFactura', { serial: folioPreview })}</div>
      ...
      <Card title={t('configuracion.metas')}>
        <MetasCard config={form} setForm={fn => setForm(f => f && fn(f))} currency={form.currency} />
      </Card>
...
function MonedasCard({ config, setForm }: { config: Config; setForm: (fn: (f: Config) => Config) => void }) {
  ...
  const base = config.currency

  const toggleActvat = (code: string, on: boolean) => {
  ...
  const saveRate = (code: string) => {
    ...
    const r = parseRates(config.exchange_rates) ?? { base, date: '', rates: {} }
    setForm(f => ({ ...f, exchange_rates: JSON.stringify({ ...r, base, date: r.date, rates: { ...r.rates, [code]: v } }) }))
  ...
  const actualizar = async () => {
    ...
    try {
      const { rates: fetched, date } = await fetchExchangeRates(base)
      const r = parseRates(config.exchange_rates) ?? { base, date: '', rates: {} }
      const merged: Record<string, number> = { ...r.rates }
      for (const c of all) if (fetched[c.code]) merged[c.code] = fetched[c.code]
      setForm(f => ({ ...f, exchange_rates: JSON.stringify({ base, date, rates: merged }) }))
      toast(`Tasas actualizadas (${date})`)
    } catch (e) {
      toast((e as Error).message, 'error')
    }
  ...
  return (
    <div className="space-y-5">
      <div>
        <div className="text-xs text-muted-foreground mb-2">Moneda base (la de tus reports)</div>
        <Select value={base} onChange={v => setForm(f => ({ ...f, currency: v }))} options={all.map(c => ({ value: c.code, label: `${c.code} · ${c.symbol} · ${c.name}` }))} />
        <p className="mt-1 text-xs text-muted-foreground">Los reports y el dashboard se muestran en esta currency. Facturas, expenses y nómina pueden registrarse en otra currency y se convierten aquí.</p>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {all.map(c => {
            ...
            return (
              <button key={c.code} type="button" onClick={() => toggleActvat(c.code, !on)}
      ...
      <div className="rounded-xl border border-gray-100 bg-muted/40 p-4 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">1 {base} = cuánto de cada currency. Se actualiza con la API pública open.er-api.com y siempre puedes corregirlo a mano (sin internet).</p>
        {rates && rates.date && (
          <p className={frescas ? 'text-xs text-emerald-700' : 'text-xs text-amber-600'}>
            {frescas ? `Actualizado: ${rates.date}` : `Tasa del ${rates.date} — revisa el tipo de cambio`}
          </p>
        )}
            ...
            return (
              <div key={c.code} className="rounded-lg bg-white border border-gray-100 px-3 py-2 space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <Input className="flex-1" type="number" step="any" min={0} placeholder={rate ? String(rate) : 'Tasa'}
                ...
                <p className="text-xs text-muted-foreground">1 {base} = {rate ?? '—'} {c.code}</p>
              </div>
            )
          })}
          {active.filter(c => c.code !== base).length === 0 && <p className="text-xs text-muted-foreground">Agrega otra currency actvat para configurar su rate.</p>}
        </div>
      </div>

      <div>
        <div className="text-xs text-muted-foreground mb-2">Agregar currency personalizada</div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">Útil si tu país no está en la lista. La rate se configura arriba.</p>
      </div>
    </div>
  )
}
...
function TasaDiaCard({ config, setForm, onSaved }: { config: Config; setForm: (fn: (f: Config) => Config) => void; onSaved: () => Promise<void> | void }) {
  ...
  const actvat = config.daily_rate_active === 'true'
  ...
  // Últimas variaciones por currency (últimos 30 registros).
  const recientes = [...tasas].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 30)
  return (
    <div className="space-y-4">
      <label className="flex items-center gap-2 text-sm">
        <input type="checkbox" className="h-4 w-4" checked={actvat} onChange={toggle} />
              ...
              <tbody>
                {recientes.map(x => (
                  <tr key={x.rate_id} className="border-t border-gray-50">
                    ...
                    <td className="px-3 py-1.5">{x.base} → {x.currency}</td>
                    <td className="px-3 py-1.5 text-right tabular-nums">{Number(x.rate).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function ComisionesCard({ config, setForm }: { config: Config; setForm: (fn: (f: Config) => Config) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {numInput(t('configuracion.comisionGastos'), parsed.expenses, n => update({ expenses: n }))}
        {numInput(t('configuracion.comisionCxp'), parsed.ap, n => update({ ap: n }))}
        {numInput(t('configuracion.comisionNomina'), parsed.payroll, n => update({ payroll: n }))}
      </div>
      <p className="text-xs text-muted-foreground">Las fees por método de pago se configuran directamente en cada selector de método (ej. al registrar un payment o partial).</p>
    </div>
  )
}

/** Metas mensuales de venta (JSON en config). */
function MetasCard({ config, setForm, currency }: { config: Config; setForm: (fn: (f: Config) => Config) => void; currency: string }) {
  const { t } = useI18n()
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        <Input type="number" min={0} step="any" value={amount} onChange={e => setMonto(e.target.value)} placeholder={`Monto (${currency})`} className="w-40" />
        ...
        <ul className="text-sm divide-y divide-gray-50 rounded-xl border border-gray-100 max-h-40 overflow-auto">
          {Object.entries(metas).sort((a, b) => a[0].localeCompare(b[0])).map(([k, v]) => (
            <li key={k} className="flex justify-between px-3 py-1.5">
              <span>{k}</span><b>{v.toLocaleString()} {currency}</b>
            </li>
          ))}
        </ul>
      )}
