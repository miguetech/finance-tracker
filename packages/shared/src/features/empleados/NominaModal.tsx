...
import { tarifaHoraExtra } from '../../reports/nomina'
...
export function NominaModal({ empleado, onClose, onSave }: { empleado: Empleado; onClose: () => void; onSave: (i: NominaAvanzadaForm) => void }) {
  ...
  const sumaPagos = pagosDiv.reduce((s, p) => s + (Number(p.monto) || 0), 0)
    ...
    <Dialog open onClose={onClose} title={`${t('empleados.nomina')} — ${empleado.nombre}`}
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div><label className="text-xs text-muted-foreground">{t('reportesFin.sueldoBase')} *</label><Input type="number" min={0} step="any" value={form.monto} onChange={set('monto')} /></div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
          ...
          <div className="space-y-2">
            ...
            <div><label className="text-xs text-muted-foreground">{t('nominaAv.comisionesVenta')}</label><Input type="number" min={0} step="any" value={form.comisiones} onChange={set('comisiones')} placeholder="0" /></div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-primary/20 bg-primary-soft/30 p-3 text-sm space-y-1">
          ...
          <div className="flex justify-between"><span>{t('nominaAv.comisionesVenta')}</span><b>{formatMoney(num(form.comisiones), monedaSel)}</b></div>
            ...
            {pagosDiv.map((p, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 items-center">
                ...
                <Input className="col-span-4" type="number" min={0} step="any" value={p.monto}
                  onChange={e => setPagosDiv(l => l.map((x, idx) => idx === i ? { ...x, monto: e.target.value } : x))}
