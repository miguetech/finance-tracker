...
export function Inventario() {
  ...
  const provMap = useMemo(() => new Map(proveedores.map(p => [p.supplier_id, p])), [proveedores])
  ...
  const alertas = productos.filter(p => stockLevel(Number(p.stock), Number(p.minimum_stock)) !== 'ok' && String(p.activo) !== 'false')
    ...
    <div className="space-y-4">
            ...
            {alertas.map(p => {
              ...
              return (
                <div key={p.product_id} className="flex items-center justify-between gap-2 rounded-lg bg-white border border-red-100 px-3 py-2 text-sm">
                  <div>
                    <div className="font-medium">{p.nombre} <StockBadge stock={Number(p.stock)} minimo={Number(p.minimum_stock)} /></div>
                    <div className="text-xs text-gray-500">{t('dashboard.quedan')} {p.stock} {p.unidad || 'pieza'} · {t('inventario.minimo')} {p.minimum_stock}</div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-card overflow-hidden">
        <Table columns={[
          ...
          { key: 'costo', header: t('inventario.precioCosto'), render: r => formatMoney(Number(r.cost_price), moneda) },
          { key: 'venta', header: t('inventario.precioVenta'), render: r => formatMoney(Number(r.sale_price), moneda) },
