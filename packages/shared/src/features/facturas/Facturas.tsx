...
export function Facturas() {
  const { t } = useI18n()
  const { facturas, deleteFactura } = useFacturas({})
  ...
  const etiquetaEstado = (f: Factura) => {
    if (f.saldo <= 0) return t('states.pagada')
    if (f.saldo < f.total) return t('states.parcial')
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Select value={estado} onChange={setEstado} options={[{ value: t('states.pendiente'), label: t('states.pendiente') }, { value: t('states.parcial'), label: t('states.parcial') }, { value: t('states.pagada'), label: t('states.pagada') }]} placeholder={t('common.estado')} />
      ...
      <ConfirmDialog open={deleteId !== null} title={t('facturas.eliminarTitulo')} message={t('facturas.eliminarMensaje')}
        onConfirm={async () => { if (deleteId) { try { await deleteFactura.mutateAsync(deleteId); toast(t('facturas.eliminada')) } catch (e) { toast((e as Error).message, 'error') } } setDeleteId(null) }}
