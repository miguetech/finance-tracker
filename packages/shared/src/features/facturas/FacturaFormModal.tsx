...
interface ItemForm { product_id: string; descripcion: string; cantidad: string; unit_price: string }
...
export function FacturaFormModal({ open, onClose, onSaved }: { open: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  const [items, setItems] = useState<ItemForm[]>([{ product_id: '', descripcion: '', cantidad: '1', unit_price: '' }])
  const [error, setError] = useState('')

  useEffect(() => {
    if (open) {
      setIdCliente('')
      setItems([{ product_id: '', descripcion: '', cantidad: '1', unit_price: '' }])
  ...
  const elegirProducto = (i: number, idProducto: string) => {
    setItems(list => list.map((it, idx) => {
      ...
      const p = productos.find(pr => pr.product_id === idProducto)
  ...
  const parsedItems = items.map(it => ({ descripcion: it.descripcion, cantidad: Number(it.cantidad) || 0, unit_price: Number(it.unit_price) || 0, ...(it.product_id ? { product_id: it.product_id } : {}) }))
  ...
  const submit = () => guardar(async () => {
    ...
    if (!clienteId && clienteRapido.trim()) {
      const nuevo = await saveCliente.mutateAsync({ customer_id: uid('cli_'), nombre: clienteRapido.trim(), rfc: '', email: '', telefono: '', direccion: '', created_at: todayLocal() })
    ...
    const factura = await createFactura.mutateAsync({ customer_id: clienteId, items: validos, issue_date: todayLocal(), due_date, notas, moneda: monedaSel })
    if (pagado && isAdmin) {
      await registerPago.mutateAsync({ tipo: 'cobro', origin_id: factura.invoice_id, fecha: todayLocal(), monto: factura.total, payment_method: 'Efectivo', notas: t('facturas.pagoContado') })
    }
    onSaved()
    onClose()
  }, {
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-3">
          ...
          {items.map((it, i) => {
            ...
            const importe = (Number(it.cantidad) || 0) * (Number(it.unit_price) || 0)
            return (
              <div key={i} className="rounded-xl border border-gray-100 p-2 space-y-2 bg-muted/20">
                ...
                <div className="flex items-center gap-2">
                  <SearchSelect className="flex-1"
                    ...
                    options={productos.filter(p => p.activo !== 'false').map(p => ({ value: p.product_id, label: `${p.nombre}${p.unidad ? ` (${p.stock} ${p.unidad})` : ''}` }))}
          ...
          <Button variant="outline" onClick={() => setItems(l => [...l, { product_id: '', descripcion: '', cantidad: '1', unit_price: '' }])}>+ {t('facturas.agregarConcepto')}</Button>
