...
export function PagoModal({ origen, onClose }: { origen: { id: string; tipo: 'cobro' | 'abono'; saldo: number; moneda?: string }; onClose: () => void }) {
  ...
  const monedaDoc = origen.moneda || config?.moneda || 'USD'
  ...
  const submit = async () => {
    ...
    if (equivalente > origen.saldo + 0.009) {
      return setError(`${t('pago.superaSaldo')} (${formatMoney(origen.saldo, monedaDoc)})`)
    }
    setGuardando(true)
    try {
  ...
  return (
    <Dialog open onClose={onClose} title={origen.tipo === 'cobro' ? t('facturas.registrarCobro') : t('cuentas.registrarAbono')}
      ...
      <div className="space-y-3">
        <p className="text-sm">{t('pago.saldoDisponible')} <b>{formatMoney(origen.saldo, monedaDoc)}</b>
        ...
        {comisionAbierta && (
          <ComisionMetodoForm metodo={metodo} actual={comisionActual} monedaBase={config?.moneda ?? 'USD'} />
        )}
        {(comisionActual.pct || comisionActual.minimo_fijo) && n > 0 && (
          <p className="text-xs text-amber-700">{t('pago.comisionPreview')}: ≈ {formatMoney(comisionEstimada, config?.moneda ?? 'USD')}</p>
        )}
        <div>
          ...
          {distinta && n > 0 && (
            <p className="mt-1 flex items-center gap-1 text-xs text-emerald-700">
              ...
              {rates?.fecha && <span className={frescas ? '' : 'text-amber-600'}>({rates.fecha})</span>}
            </p>
          )}
        </div>
 ...
 *  (independiente de la configuración global de comisiones por módulo). */
