...
export function Clientes() {
  const { t } = useI18n()
  const { customers, saveCustomer, deleteCustomer } = useClientes()
  ...
  return (
    <div className="space-y-4">
          ...
          {canEdit('customers') && (
            <>
              ...
              <ImportarClientes onImportados={async cs => {
                let n = 0
                for (const c of cs) {
                  const existe = customers.some(x => x.name.trim().toLowerCase() === String(c.name).trim().toLowerCase())
                  if (existe) continue
                  await saveCustomer.mutateAsync({ ...c, customer_id: uid('cli_'), name: String(c.name), rfc: '', created_at: todayLocal() } as Cliente)
                  n++
                }
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-card overflow-hidden">
        <Table
          columns={[
            { key: 'nombre', header: t('common.nombre'), render: c => String(c.name) },
            { key: 'rfc', header: docLabel, render: c => String(c.rfc) },
            { key: 'email', header: t('common.email'), render: c => String(c.email) },
            { key: 'telefono', header: t('common.telefono'), render: c => String(c.phone) },
            { key: 'acciones', header: '', render: c => (
              <div className="flex gap-2">
                {canEdit('customers') && (<Button variant="ghost" icon={<IconEdit className="w-4 h-4" />} onClick={() => { setEditando(c as unknown as Cliente); setFormOpen(true) }}>{t('common.editar')}</Button>)}
      ...
      <ClienteFormModal open={formOpen} onClose={() => setFormOpen(false)} initial={editando}
        onSave={async c => {
          try { await saveCustomer.mutateAsync(c); toast(t('clientes.guardado')) } catch (e) { toast((e as Error).message, 'error'); throw e }
        }} />
      <ConfirmDialog open={deleteId !== null} title={t('clientes.eliminarTitulo')} message={t('clientes.eliminarMensaje')}
        onConfirm={async () => {
          if (!deleteId) return
          try { await deleteCustomer.mutateAsync(deleteId); toast(t('clientes.eliminado')) } catch (e) { toast((e as Error).message, 'error') }
          setDeleteId(null)
