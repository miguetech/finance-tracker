...
export function Proveedores() {
  const { t } = useI18n()
  const { suppliers, saveSupplier, deleteSupplier } = useProveedores()
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex gap-2">
          {canEdit('suppliers') && <ImportarContactos onImportados={async (nuevos) => {
            for (const c of nuevos) {
              await saveSupplier.mutateAsync({ supplier_id: uid('prov_'), name: c.name, rfc: '', email: c.email, phone: c.phone, address: '', created_at: todayLocal() })
            }
          }} />}
          {canEdit('suppliers') && (<Button icon={<IconPlus className="w-4 h-4" />} onClick={() => { setEditando(null); setFormOpen(true) }}>{t('proveedores.nuevo')}</Button>)}
        </div>
      </div>
      <div className="bg-white border border-gray-200 rounded-xl shadow-card overflow-hidden">
        <Table columns={[
          { key: 'nombre', header: t('common.nombre'), render: r => String(r.name) },
          { key: 'rfc', header: docLabel, render: r => String(r.rfc) },
          { key: 'email', header: t('common.email'), render: r => String(r.email) },
          { key: 'telefono', header: t('common.telefono'), render: r => String(r.phone) },
          { key: 'acciones', header: '', render: r => {
            const prov = r as unknown as Proveedor
            return (
              <div className="flex gap-1.5 items-center flex-wrap">
                <a href={whatsappUrl(prov.phone, `Hola ${prov.name}, `)} target="_blank" rel="noreferrer"
                  title={prov.phone ? t('whatsapp.abrirWhatsApp') : t('whatsapp.sinTelefono')}>
                  <Button size="sm" variant="success" disabled={!prov.phone}>{t('whatsapp.abrirWhatsApp')}</Button>
                </a>
                <a href={mailtoUrl(prov.email, '')} title={t('whatsapp.abrirEmail')}>
                  <Button size="sm" variant="outline" disabled={!prov.email}>{'✉'}</Button>
                </a>
                {canEdit('suppliers') && (<Button variant="ghost" icon={<IconEdit className="w-4 h-4" />} onClick={() => { setEditando(prov); setFormOpen(true) }}>{t('common.editar')}</Button>)}
        ...
        {suppliers.length === 0 && <p className="p-4 text-sm text-gray-500">{t('proveedores.sinProveedores')}</p>}
      </div>
      <ProveedorFormModal open={formOpen} onClose={() => setFormOpen(false)} initial={editando}
        onSave={async p => { try { await saveSupplier.mutateAsync(p); toast(t('proveedores.guardado')) } catch (e) { toast((e as Error).message, 'error'); throw e } }} />
      <ConfirmDialog open={deleteId !== null} title={t('proveedores.eliminarTitulo')} message={t('proveedores.eliminarMensaje')}
        onConfirm={async () => { if (deleteId) { try { await deleteSupplier.mutateAsync(deleteId); toast(t('proveedores.eliminado')) } catch (e) { toast((e as Error).message, 'error') } } setDeleteId(null) }} onClose={() => setDeleteId(null)} />
    </div>
  )
}
          ...
          <ul className="max-h-56 overflow-auto divide-y divide-gray-50 rounded-xl border border-gray-100 text-sm">
            {preview.map((c, i) => (
              <li key={i} className="px-3 py-1.5 flex justify-between gap-2">
                ...
                <span className="text-xs text-muted-foreground shrink-0">{c.phone || c.email || ''} · {t('contactos.origenEtiqueta', { origen: 'CSV' })}</span>
              </li>
            ))}
          </ul>
        </div>
