 ...
 *  a la currency del documento y balance restante acumulado. */
export function HistorialAbonos({ pagos, totalDoc, monedaDoc }: {
  ...
  const ordenados = [...pagos].sort((a, b) => String(a.date).localeCompare(String(b.date)))
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-xs uppercase text-muted-foreground text-left">
            ...
            {ordenados.some(p => (p.currency || monedaDoc) !== monedaDoc) && (
        ...
        <tbody>
          {ordenados.map(p => {
            const monedaPago = p.currency || monedaDoc
            const convertido = convert(Number(p.amount) || 0, monedaPago, monedaDoc, config)
            restante = Math.round((restante - convertido) * 100) / 100
            return (
              <tr key={p.payment_id} className="border-b border-gray-50">
                <td className="py-1 whitespace-nowrap">{String(p.date)}</td>
                <td className="py-1">{p.payment_method}</td>
                <td className="py-1 text-right tabular-nums">
                  {formatMoney(Number(p.amount), monedaPago)}
                  {monedaPago !== monedaDoc && <span className="text-xs text-gray-400"> ({monedaPago})</span>}
                </td>
                {ordenados.some(x => (x.currency || monedaDoc) !== monedaDoc) && (
