...
function TabProductoIndividual({ data, desde, hasta }: { data: InvData; desde: string; hasta: string }) {
  ...
  const totalUnidades = ventas.reduce((s, v) => s + v.quantity, 0)

  return (
    <div className="space-y-4">
      <Card title={t('reportesFin.tabProducto')}>
        <SearchSelect value={idProducto} onChange={setIdProducto}
          options={activos.map(p => ({ value: p.product_id, label: `${p.name}${p.category ? ` · ${p.category}` : ''}` }))}
      ...
      {idProducto && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard label={t('reportesFin.stockActual')} value={`${producto ? Number(producto.stock).toLocaleString() : '—'} ${producto?.unit ?? ''}`} />
            <StatCard label={t('reportesFin.unidadesVendidas')} value={totalUnidades.toLocaleString()} />
            <StatCard label={t('reportes.facturado')} value={formatMoney(totalIngresos, currency)} tone="positive" />
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-muted text-left text-xs uppercase text-muted-foreground">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right">{t('facturas.importe')} ({currency})</th>
                </tr>
              </thead>
              <tbody>
                {ventas.map((v, i) => (
                  <tr key={i} className="border-t border-gray-100 hover:bg-muted/50">
                    ...
                    <td className="px-4 py-2">{v.serial}</td>
                    <td className="px-4 py-2 max-w-40 truncate" title={v.cliente}>{v.cliente}</td>
                    <td className="px-4 py-2 text-right tabular-nums">{v.quantity}</td>
                    <td className="px-4 py-2 text-right tabular-nums">{formatMoney(v.unit_price, currency)}</td>
                    <td className="px-4 py-2 text-right tabular-nums font-medium">{formatMoney(v.importe_base, currency)}</td>
                  </tr>
                ))}
          ...
          {ventas.length > 0 && (
            <div className="flex justify-end">
              <Button size="sm" variant="outline" onClick={() => exportCSV(`ventas_${producto?.name ?? idProducto}_${desde}_${hasta}`, ventas, [
                { key: 'fecha', header: t('common.fecha') }, { key: 'folio', header: t('facturas.folio') }, { key: 'cliente', header: t('facturas.cliente') },
                { key: 'cantidad', header: t('facturas.cant') }, { key: 'unit_price', header: t('facturas.precio') }, { key: 'importe_base', header: `${t('facturas.importe')} (${currency})` }
...
function TabStockBajo({ data, hasta }: { data: InvData; hasta: string }) {
  ...
  const pdfStock = () => exportPDF(t('reportesFin.stockBajoTitulo'), {
    ...
    tablas: [{
      ...
      numericas: [2, 3, 4],
      filas: lista.map(p => [p.name, p.category, p.stock, p.minimum_stock, p.faltante, p.unit, p.supplier_name] as (string | number)[])
    }]
  })
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <StatCard label={`${t('reportesFin.faltante')} total`} value={`${lista.reduce((s, p) => s + p.faltante, 0).toLocaleString()} ${lista[0]?.unit ?? ''}`} />
      </div>
      <div className="overflow-x-auto rounded-xl border border-gray-100 shadow-card">
        <table className="min-w-full text-sm">
          <thead className="bg-muted text-left text-xs uppercase text-muted-foreground">
          ...
          <tbody>
            {lista.map(p => (
              <tr key={p.product_id} className="border-t border-gray-100 hover:bg-muted/50">
                ...
                <td className="px-4 py-2 text-right tabular-nums">{p.stock.toLocaleString()} {p.unit}</td>
                ...
                <td className="px-4 py-2">
                  {p.supplier_name && (
                    <a className="text-xs text-emerald-700 hover:underline" target="_blank" rel="noreferrer"
                      href={`https://wa.me/?text=${encodeURIComponent(`Hola ${p.supplier_name}, necesito reordenar: ${p.faltante} ${p.unit} de ${p.name}. Gracias.`)}`}>
...
function TabMultiproducto({ data, desde, hasta }: { data: InvData; desde: string; hasta: string }) {
  ...
  const filtrados = useMemo(() => {
    const base = q.trim() ? products.filter(p => p.name.toLowerCase().includes(q.trim().toLowerCase()) || p.category.toLowerCase().includes(q.trim().toLowerCase())) : products
        ...
        <>
              ...
              <tbody>
                {stats.map(s => (
                  <tr key={s.product_id} className="border-t border-gray-100 hover:bg-muted/50">
                    <td className="px-4 py-2 font-medium max-w-40 truncate" title={s.name}>{s.name}</td>
                    <td className="px-4 py-2 text-right tabular-nums">{s.unidades_vendidas.toLocaleString()} {s.unit}</td>
                    <td className="px-4 py-2 text-right tabular-nums">{formatMoney(s.ingresos, currency)}</td>
                    <td className="px-4 py-2 text-right tabular-nums">{formatMoney(s.margen_unitario, currency)}</td>
                    ...
                    <td className="px-4 py-2 text-right tabular-nums">{s.velocidad_out}</td>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <Card title={t('reportesFin.contribucion')}>
              <DonutChart data={stats.map(s => ({ label: s.name, value: s.contribucion_pct }))} format={(n) => `${n}%`} />
            </Card>
            <Card title={t('reportesFin.margenPct')}>
              <HBarChart data={stats.map(s => ({ label: s.name, value: s.margen_pct }))} format={(n) => `${n}%`} color="#8b5cf6" />
            </Card>
            <Card title={t('reportesFin.velocidadSalida')}>
              <HBarChart data={stats.map(s => ({ label: s.name, value: s.velocidad_out }))} format={(n) => String(n)} color="#f59e0b" />
            </Card>
          </div>
          ...
          <div className="flex justify-end gap-2">
            <Button size="sm" variant="outline" onClick={() => exportPDF(t('reportesFin.tabMultiproducto'), {
              ...
              tablas: [{
                ...
                numericas: [1, 3, 4, 5],
                filas: stats.map(s => [s.name, s.unidades_vendidas, s.ingresos, s.margen_pct, s.rotacion, s.contribucion_pct] as (string | number)[])
              }]
...
/** Reporte de ventas rápidas (mayor velocidad de out en el período). */
export function VentasRapidasCard({ data }: { data?: InvData }) {
  const { t } = useI18n()
  if (!data) return null
  const rapidas = [...data.statsProductos].filter(s => s.velocidad_out > 0).sort((a, b) => b.velocidad_out - a.velocidad_out).slice(0, 10)
  if (rapidas.length === 0) return null
  return (
    <Card title={t('reportesFin.ventasRapidasTitulo')}>
      <HBarChart data={rapidas.map(s => ({ label: s.name, value: s.velocidad_out }))} format={(n) => `${n}/día`} color="#06b6d4" />
    </Card>
  )
}
