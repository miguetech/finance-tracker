...
export function createRepository(ctx: RepoContext) {
  ...
  const mapaAnioFacturas = async (): Promise<Map<string, string>> => {
    ...
    for (const f of facturas) {
      ...
      if (anio && f.invoice_id) m.set(f.invoice_id, anio)
    }
    return m
  }
  ...
  async function anioDeItem(fila: Record<string, unknown>): Promise<string> {
    const idPadre = String(fila.invoice_id ?? '')
    if (!idPadre) return ''
...
function invalidarSistemaCache(): void {
  ...
  async function crearHojaEventos(anio: string): Promise<string> {
    ...
    // Nombre ESTABLE: no depende de company_name (que cambia en Config).
    // La personalización se hace vía Config.nombreBaseHoja (opcional, solo UI).
    const base = (cfg?.baseSheetName ?? 'FinanceTracker').trim()
  ...
  async function hacerRehome(): Promise<{ movidos: number }> {
    ...
    for (const f of facturas) {
      ...
      if (a && f.invoice_id) padre.set(f.invoice_id, a)
    }
    ...
    for (const src of fuentes) {
      for (const r of src.rows) {
        const idPadre = String(r.invoice_id ?? '')
  ...
  async function reemplazarEventoFragmentado<T extends object>(t: keyof typeof TABLES, filas: T[]): Promise<void> {
    ...
    for (const f of filas) {
      ...
      const anio = mapItems
        ? (mapItems.get(String(r.invoice_id ?? '')) ?? '')
  ...
  async function descontarStock(idProducto: string, cantidad: number, motivo: string): Promise<void> {
    ...
    const prod = productos.find(p => p.product_id === idProducto)
    if (!prod) return
    const nuevoStock = Math.max(0, (Number(prod.stock) || 0) - cantidad)
    await replaceTable('Productos', productos.map(r => (r.product_id === idProducto ? { ...r, stock: nuevoStock } : r)))
    await appendRows('Movimientos_Stock', [{ movement_id: uid('mov_'), product_id: idProducto, tipo: 'salida', cantidad, motivo, supplier_id: '', fecha: todayLocal() }])
  }
...
/** Elimina el archivo de un año (a papelera) y lo marca como borrado para
  ...
  /** Panel Almacenamiento: dónde vive cada cosa (ids de Drive). */
     ...
     *  Cubre Productos.imagen y Config.company_logo. Idempotente: salta lo
    ...
    async migrarImagenesADrive(): Promise<{ migradas: number; fallidas: number }> {
      ...
      for (const p of productos) {
        ...
        try {
          const url = await subir(img, `producto_${p.product_id}_mig.png`)
      ...
      try {
        const cfg = await readConfig()
        const logo = String(cfg.company_logo ?? '')
        if (logo.startsWith('data:image')) {
          const url = await subir(logo, `logo_migrado_${Date.now()}.png`)
          await writeConfig({ ...cfg, company_logo: url })
          migradas++
        }
      } catch { fallidas++ }
    ...
    async saveConfig(config: Config): Promise<void> {
      ...
      if (parsed.daily_rate_active === 'true' && parsed.exchange_rates) {
        try {
          const r = parseRates(parsed.exchange_rates)
    ...
    async saveCliente(cliente: Cliente): Promise<Cliente> {
      ...
      const saved = { ...parsed, customer_id: parsed.customer_id || uid('cli_'), created_at: parsed.created_at || todayLocal() } as unknown as Cliente
      await insertOrReplace('Clientes', 'customer_id', saved)
      return saved
    },
    ...
    async deleteCliente(id: string): Promise<void> {
      ...
      const all = (await readTable('Clientes')).filter(r => r.customer_id !== id)
    ...
    async saveCodigo(input: Partial<CodigoAcceso>): Promise<CodigoAcceso> {
      ...
      const codigo = input.codigo || generarCodigo(prefijoDesdeNombre((await readConfig()).company_name), new Date().getFullYear(), existentes.map(c => String(c.codigo)))
    ...
    async renovarCodigo(codigo: string, nuevaExpira: string): Promise<CodigoAcceso> {
      ...
      const updated: CodigoAcceso = { ...actual, expires_at: nuevaExpira, activo: 'true', usos: actual.max_uses }
    ...
    async createFactura(input: { customer_id: string; items: { descripcion: string; cantidad: number; unit_price: number; product_id?: string }[]; issue_date: string; due_date: string; notas: string; moneda?: string }): Promise<Factura> {
      const parsed = FacturaInputSchema.parse(input)
      const clientes = await readTable('Clientes')
      const cliente = clientes.find(c => c.customer_id === parsed.customer_id)
      if (!cliente) throw new Error('Cliente no existe')
      const cfg = await readConfig()
      const moneda = parsed.moneda || cfg.moneda
      const { items, totals } = buildFactura(parsed.items, cfg.vat_percent, getCurrency(moneda).decimals)
      ...
      const invoice_id = (input as { invoice_id?: string }).invoice_id || uid('fac_')
      const previa = (await readTable<Factura>('Facturas')).find(f => f.invoice_id === invoice_id)
      if (previa) return previa
      // Validar stock disponible antes de escribir nada (items con producto vinculado).
      const itemsConProducto = items.filter(it => it.product_id)
      ...
      const folio = await withMutex<string>(
        ...
        async () => {
          ...
          await writeConfig({ ...c, serial_counter: c.serial_counter + 1 })
          return `${expandFolioTemplate(c.serial_prefix, parsed.issue_date)}${String(folioN).padStart(3, '0')}`
        }
      )
      ...
      const itemRows = items.map(it => ({ invoice_id, ...it }))
      await appendRows('Factura_Items', itemRows)
      // Descuento automático de inventario por ventas.
      for (const it of itemsConProducto) {
        await descontarStock(String(it.product_id), it.cantidad, `Venta ${folio}`)
      }
      return factura
    },
    ...
    async getFactura(id: string): Promise<{ factura: Factura; items: FacturaItem[] }> {
      ...
      const items = (await readTable<FacturaItem & { invoice_id: string }>('Factura_Items')).filter(i => i.invoice_id === id).map(i => ({
    ...
    async updateFactura(id: string, input: { customer_id: string; items: { descripcion: string; cantidad: number; unit_price: number; product_id?: string }[]; issue_date: string; due_date: string; notas: string; moneda?: string }): Promise<Factura> {
      const parsed = FacturaInputSchema.parse(input)
      const facturas = await readTable<Factura>('Facturas')
      ...
      const oldItems = (await readTable('Factura_Items')).filter(i => i.invoice_id !== id)
      await replaceTable('Facturas', facturas.map(f => (f.invoice_id === id ? updated : f)))
      await replaceTable('Factura_Items', [...oldItems, ...items.map(it => ({ invoice_id: id, ...it }))])
      return updated
    },

    async deleteFactura(id: string): Promise<void> {
      const all = (await readTable('Facturas')).filter(r => r.invoice_id !== id)
      await replaceTable('Facturas', all)
      await replaceTable('Factura_Items', (await readTable('Factura_Items')).filter(r => r.invoice_id !== id))
      await replaceTable('Pagos', (await readTable('Pagos')).filter(r => r.origin_id !== id))
    },
    ...
    async saveGasto(gasto: Gasto): Promise<Gasto> {
      ...
      const cfg = await readConfig()
      const moneda = parsed.moneda || cfg.moneda
      const saved = { ...parsed, expense_id: parsed.expense_id || uid('gas_'), moneda, exchange_rate: parsed.exchange_rate || tipoCambioDe(cfg, moneda) } as unknown as Gasto
      await insertOrReplace('Gastos', 'expense_id', saved)
      return saved
    },

    async deleteGasto(id: string): Promise<void> {
      await replaceTable('Gastos', (await readTable('Gastos')).filter(r => r.expense_id !== id))
    },
    ...
    async saveProveedor(p: Proveedor): Promise<Proveedor> {
      ...
      const saved = { ...parsed, supplier_id: parsed.supplier_id || uid('prov_'), created_at: parsed.created_at || todayLocal() } as unknown as Proveedor
      await insertOrReplace('Proveedores', 'supplier_id', saved)
      return saved
    },
    ...
    async deleteProveedor(id: string): Promise<void> {
      ...
      await replaceTable('Proveedores', (await readTable('Proveedores')).filter(r => r.supplier_id !== id))
    },
    ...
    async saveEmpleado(emp: Empleado): Promise<Empleado> {
      ...
      const saved = { ...parsed, employee_id: parsed.employee_id || uid('emp_'), hire_date: parsed.hire_date || todayLocal() } as unknown as Empleado
      await insertOrReplace('Empleados', 'employee_id', saved)
      return saved
    },

    async listAsistencias(filtro: { employee_id?: string; desde?: string; hasta?: string } = {}): Promise<Asistencia[]> {
    ...
    async saveAsistencia(input: Omit<Asistencia, 'attendance_id' | 'employee_name'> & { attendance_id?: string; employee_name?: string }): Promise<Asistencia> {
      const parsed = AsistenciaSchema.parse(input)
      const empleados = await readTable<Empleado>('Empleados')
      const emp = empleados.find(e => e.employee_id === parsed.employee_id)
      const saved: Asistencia = {
        ...parsed,
        attendance_id: parsed.attendance_id || uid('asi_'),
        employee_name: emp?.nombre ?? parsed.employee_name ?? ''
      }
      ...
      const existente = all.find(r => r.employee_id === saved.employee_id && String(r.fecha) === saved.fecha && r.attendance_id !== saved.attendance_id)
      if (existente) {
        await replaceTable('Asistencias', all.map(r => (r.attendance_id === existente.attendance_id ? (saved as unknown as Record<string, string | number>) : r)))
      } else {
        await insertOrReplace('Asistencias', 'attendance_id', saved)
      }
      return saved
    },

    async deleteAsistencia(id: string): Promise<void> {
      const all = (await readTable('Asistencias')).filter(r => r.attendance_id !== id)
      await replaceTable('Asistencias', all)
    },

    async deleteEmpleado(id: string): Promise<void> {
      const gastos = await readTable('Gastos')
      const emp = (await readTable('Empleados')).find(r => r.employee_id === id)
      ...
      await replaceTable('Empleados', (await readTable('Empleados')).filter(r => r.employee_id !== id))
    },

    async registerNomina(input: { employee_id: string; mes: string; monto: number; payment_method: MetodoPago; fecha: string; notas: string; moneda?: string }): Promise<Gasto> {
      const parsed = NominaInputSchema.parse(input)
      const emp = (await readTable('Empleados')).find(r => r.employee_id === parsed.employee_id)
    ...
    async createCxp(input: { supplier_id: string; document_serial: string; categoria: string; descripcion: string; issue_date: string; due_date: string; total_amount: number; notas: string; moneda?: string }): Promise<CuentaPagar> {
      const parsed = CxpInputSchema.parse(input)
      const provs = await readTable('Proveedores')
      const prov = provs.find(p => p.supplier_id === parsed.supplier_id)
      ...
      const cxp: CuentaPagar = {
        ...
        supplier_name: String(prov.nombre),
        document_serial: parsed.document_serial,
        categoria: parsed.categoria,
        descripcion: parsed.descripcion,
        issue_date: parsed.issue_date,
        due_date: parsed.due_date,
        total_amount: round2(parsed.total_amount),
        saldo: round2(parsed.total_amount),
        estado: 'pendiente',
        notas: parsed.notas,
        moneda,
        exchange_rate: tipoCambioDe(cfg, moneda)
    ...
    async deleteCxp(id: string): Promise<void> {
      await replaceTable('Cuentas_Pagar', (await readTable('Cuentas_Pagar')).filter(r => r.ap_id !== id))
      await replaceTable('Pagos', (await readTable('Pagos')).filter(r => r.origin_id !== id))
    },

    async registerPago(pago: { tipo: 'cobro' | 'abono'; origin_id: string; fecha: string; monto: number; payment_method: MetodoPago; notas: string }): Promise<Pago> {
      ...
      return withMutex<Pago>(mutexReadRow, mutexWriteRow, async () => {
        ...
        const target = rows.find(r => r[idKey] === parsed.origin_id)
        ...
        const tipoCambioOrigen = Number(target.exchange_rate) || 1
        ...
        const pagoRow: Pago = { payment_id: uid('pag_'), ...parsed, moneda: monedaPago, exchange_rate: tipoCambioPago }
        ...
        if (table === 'Cuentas_Pagar' && nuevoSaldo <= 0) {
          gastoGenerado = {
            ...
            descripcion: `CXP pagada ${String(target.document_serial || '')} — ${String(target.supplier_name || '')}`.trim(),
            monto: Number(target.total_amount) || 0,
            payment_method: parsed.payment_method,
            proveedor: String(target.supplier_name || ''),
            moneda: monedaOrigen,
            exchange_rate: tipoCambioOrigen
          }
        }
        ...
        const actualizar = (r: Record<string, string | number>) => {
          if (r[idKey] === parsed.origin_id) {
            if (table === 'Facturas') return { ...r, saldo: nuevoSaldo, paid_at: nuevoSaldo <= 0 ? parsed.fecha : String(r.paid_at ?? '') }
    ...
    async getCategorias(kind: 'gastos' | 'cxp'): Promise<string[]> {
      const cfg = await readConfig()
      const raw = kind === 'gastos' ? cfg.expense_categories : cfg.ap_categories
      return raw.split(',').map(s => s.trim()).filter(Boolean)
    },

    async listProductos(): Promise<Producto[]> {
      const rows = await readTable<Producto>('Productos')
      const provs = (await readTable('Proveedores')).reduce<Record<string, string>>((m, p) => { m[String(p.supplier_id)] = String(p.nombre ?? ''); return m }, {})
      return enrichNombreProveedor(rows, provs)
    },

    async saveProducto(p: Producto): Promise<Producto> {
      const parsed = ProductoSchema.parse(p)
      const provs = (await readTable('Proveedores')).reduce<Record<string, string>>((m, pr) => { m[String(pr.supplier_id)] = String(pr.nombre ?? ''); return m }, {})
      const cfg = await readConfig()
      const saved = {
        ...parsed,
        product_id: parsed.product_id || uid('prod_'),
        created_at: parsed.created_at || todayLocal(),
        moneda: parsed.moneda || cfg.moneda,
        supplier_name: parsed.supplier_name || provs[String(parsed.supplier_id)] || ''
      } as unknown as Producto
      await insertOrReplace('Productos', 'product_id', saved)
      return saved
    },

    async deleteProducto(id: string): Promise<void> {
      const all = (await readTable('Productos')).filter(r => r.product_id !== id)
      await replaceTable('Productos', all)
    },

    async registrarMovimiento(input: { product_id: string; tipo: TipoMovimiento; cantidad: number; motivo: string; supplier_id: string; fecha: string }): Promise<MovimientoStock> {
      const parsed = MovimientoStockSchema.parse(input)
      return withMutex<MovimientoStock>(mutexReadRow, mutexWriteRow, async () => {
        const productos = await readTable<Producto>('Productos')
        const prod = productos.find(p => p.product_id === parsed.product_id)
        ...
        const idProveedor = parsed.tipo === 'entrada' ? (parsed.supplier_id || String(prod.supplier_id || '')) : ''
        ...
        await replaceTable('Productos', (productos).map(r => (r.product_id === parsed.product_id ? { ...r, stock: nuevoStock, supplier_id: idProveedor || String(r.supplier_id ?? '') } : r)))
    ...
    async listGastosFijos(): Promise<GastoFijo[]> {
      ...
      const provs = (await readTable('Proveedores')).reduce<Record<string, string>>((m, p) => { m[String(p.supplier_id)] = String(p.nombre ?? ''); return m }, {})
      return rows.map(r => ({ ...r, supplier_name: r.supplier_name || provs[String(r.supplier_id)] || '' }))
    },

    async saveGastoFijo(gf: GastoFijo): Promise<GastoFijo> {
      const parsed = GastoFijoSchema.parse(gf)
      const saved = { ...parsed, fixed_expense_id: parsed.fixed_expense_id || uid('gfx_') } as unknown as GastoFijo
      await insertOrReplace('Gastos_Fijos', 'fixed_expense_id', saved)
      return saved
    },

    async deleteGastoFijo(id: string): Promise<void> {
      await replaceTable('Gastos_Fijos', (await readTable('Gastos_Fijos')).filter(r => r.fixed_expense_id !== id))
    },
    ...
    async registrarTasa(input: Omit<TasaHistorial, 'rate_id'>): Promise<TasaHistorial> {
      ...
      const saved: TasaHistorial = { ...parsed, rate_id: uid('tasa_') }
    ...
    async registerNominaAvanzada(input: NominaAvanzadaInput): Promise<{ gasto: Gasto; detalle: NominaDetalle }> {
      ...
      if (parsed.split_payments.length > 0) {
        const suma = round2(parsed.split_payments.reduce((s, p) => s + p.monto, 0))
      ...
      const gasto = await this.registerNomina({
        ...
        mes: parsed.mes,
        monto: parsed.monto,
        payment_method: parsed.split_payments.length > 0 ? parsed.split_payments.map(p => `${p.payment_method}: ${p.monto}`).join(', ') : parsed.payment_method,
        fecha,
        notas: parsed.notas,
        moneda
      })
      const detalle: NominaDetalle = {
        ...
        employee_id: parsed.employee_id,
        mes: parsed.mes,
        base_salary: round2(parsed.base_salary),
        horas_extra: round2(parsed.horas_extra),
        overtime_rate: round2(parsed.overtime_rate),
        overtime_amount: round2(parsed.horas_extra * parsed.overtime_rate),
        bonos: round2(parsed.bonos),
        comisiones: round2(parsed.comisiones),
        total: round2(parsed.monto),
        moneda,
        payment_method: parsed.payment_method,
        split_payments: parsed.split_payments.length > 0 ? JSON.stringify(parsed.split_payments) : '',
        fecha,
    ...
    async getReporteFinanciero(rango: RangoFecha): Promise<{
      ...
      const [facturas, gastos, cxps, pagos, productos, items] = await Promise.all([
        ...
        readTable<FacturaItem & { invoice_id: string }>('Factura_Items')
      ])
    ...
    async getReportesInventario(rango: RangoFecha, idsProductos?: string[]): Promise<{
      ...
      const items = (tablasInv.Factura_Items ?? []) as unknown as (FacturaItem & { invoice_id: string })[]
      const ids = idsProductos?.length ? idsProductos : productos.filter(p => p.activo !== 'false').map(p => p.product_id)
      return {
    ...
    async listFacturasItems(): Promise<(FacturaItem & { invoice_id: string })[]> {
      return readTable<FacturaItem & { invoice_id: string }>('Factura_Items')
    },
    ...
    async getVentasProducto(idProducto: string, rango: RangoFecha): Promise<VentaProductoFila[]> {
      ...
      return historialVentasProducto(
        (t.Factura_Items ?? []) as unknown as (FacturaItem & { invoice_id: string })[],
    ...
    async getMetasVsLogros(meses: string[]) {
      ...
      return metasVsLogros((tablasMeta.Facturas ?? []) as unknown as Factura[], parseMetas(cfg.monthly_goals), meses)
    }
  }
}
